...
<!-- Show a NoNodeError -->
<%def name="divNoNodeError(psText)">
  <div class="alert alert-error">
    <i class="fa fa-exclamation-triangle"></i>
     ${ _('Please contact your administrator to solve this:') }
     <ul>
        <li>${ _('Error connecting to zookeper server(s).') } <b> ${psText} </b> </li>        
     </ul>
  </div>
</%def>

<!-- Show an error with zookeepers -->
<%def name="divConnectionError(psText)">
  <div class="alert alert-error">
    <i class="fa fa-exclamation-triangle"></i>
    ${ _('Please contact your administrator to solve this.') }
    <ul>
        <li>${ _('The node does not exist or the path is wrong:') } <b> ${psText} </b> </li>        
     </ul>
  </div>    
</%def>
         ...
         <form id="frmCreateTopic" method="post" enctype="multipart/form-data" action="/kafka/_create_topic/">
            ...
            <div class="modal-content">
               ...
               <div class="modal-footer">  
                  ...
                  <img id="imgLoading" src="${ static('kafka/art/spinner.gif') }" class="hide"/>
               </div>
            </div>   
         </form>      
      </div>
